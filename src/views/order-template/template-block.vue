    ...
    <table>
      <tbody>
        <tr class="edit-block-head">
          <td>字段名</td><td>字段名长度</td><td>字段内容长度</td><td>类型</td><td>字段名位置/行</td><td>字段名位置/列</td><td></td>
        </tr>
        <tr
          v-for="(item, index) in blockData"
          :key="`${item.col}${item.row}${index}`">
          <td class="edit-name">
            <input v-if="!item.isCustom" type="text" v-model="item.name">
          ...
          <td class="edit-row">
            <input type="number" v-model="item.row">
          </td>
          <td class="edit-col">
            <input type="number" v-model="item.col">
          </td>
